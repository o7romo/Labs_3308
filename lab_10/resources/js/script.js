...
$(document).ready(function() {
  ...
  $.ajax({url:url, dataType:"jsonp"}).then(function(data) {
    ...
    */
      document.getElementById("image_today").innerHTML = data.current.weather_icons;
      ...
      currentTemp = getFarenheitTemp(currentTemp);
    ...
    */
    ...
    for(var i = 0; i < 6; i++){
      ...
      console.log(day);
      var week_names = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday','Sunday'];
      dayOfWeek=week_names[new Date(day.date).getDay()];
      var max_temp = getFarenheitTemp(day.maxtemp);
      var min_temp = getFarenheitTemp(day.mintemp);
      if(i >= 1)
      {
        var card = `<div style="width: 20%;">
        <div class="card">
          <div class="card-body">
            <h5 class="card-title">${dayOfWeek}<!-- List Day of the Week Here --></h5>
            <p class="card-text">High: ${max_temp + " F"}<!--List Temperature High --> <br>
              Low: ${min_temp}<!-- List Temperature Low --><br>
              Sunrise: ${day.astro.sunrise}<!-- List Time of Sunrise --><br>
              Sunset: ${day.astro.sunset}<!-- List Time of Sunset --></p>
          </div>
        </div>
          </div>`
        document.getElementById("5_day_forecast").innerHTML += card;
      }
    }
  })
});
